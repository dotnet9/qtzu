...
AO_SAMPLES = 20
AO_DISTANCE = 1.15
AO_STRENGTH = 1.15      # 0.9→1.15：接触阴影更深（与折缝压暗配合，体积感才出得来）
AO_TINT = (0.60, 0.53, 0.46)   # 冷紫灰 → 暖褐：粘土手办的暗部是暖的
...
# 三角面上限（整数）。check_budget 直接拿它比 info["tri"]，所以只能放整数。
# 依据：
#   pet 3500→8000：词宠图元化 + 按角度拆边后实测 1700~6360 面（细分已从球 18x14 压到 12x9）
