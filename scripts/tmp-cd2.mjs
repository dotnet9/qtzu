...
const anchor = '// 城市岛地面贴图：草底 + 城市色分区（路网已按需求移除，绿化走 3D 树草）' + NL + 'function cityIslandTexture(color, level, shape) {';
if (!c.includes(anchor)) { console.log('anchor missing'); process.exit(1); }
c = c.replace(anchor, texFn + NL + anchor);
...
const callOld = 'cityIslandTexture(color, isl.level, pts)';
c = c.replace(callOld, 'cityIslandTexture(color, isl.level, pts, isl)');
writeFileSync(p, c);
console.log('chengdu texture applied cleanly');