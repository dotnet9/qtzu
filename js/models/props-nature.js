...
export const tree = (blossom = false) => {
  const g = G();
  // 树干：两段收分（根部外扩）+ 两条一级枝，别再用一根等粗圆柱（参考图里树是"有收分"的）
  cyl(g, 0.2, 0.26, 0.34, '#8A6242', 0, 0.17, 0, 0, 0, 0, 12);
  cyl(g, 0.13, 0.19, 1.5, '#A87551', 0, 1.0, 0, 0, 0, 0, 12);
  cyl(g, 0.07, 0.1, 0.62, '#9A6B4A', 0.22, 1.72, 0.06, 0, 0, -0.5, 8);
  cyl(g, 0.06, 0.09, 0.55, '#9A6B4A', -0.2, 1.78, -0.08, 0.38, 0, 0.46, 8);
  // 树冠：暗（底部/背光）→ 中（主体）→ 亮（顶部受光）三色阶，全部刻面，读得出"一团一团的叶"
  const dark = blossom ? '#D9739E' : '#4E8A4E';
  const mid = blossom ? '#F0A6C4' : '#6FBF73';
  const lite = blossom ? '#FFCFE0' : '#A8E1A2';
  const hi = blossom ? '#FFF0F6' : '#CDF2BC';
  const F = { flat: true };
  sph(g, 0.62, dark, -0.34, 1.72, -0.3, 1.05, 1, 1, 0, 0, 0, F);
  sph(g, 0.56, dark, 0.3, 1.66, 0.34, 1, 1.02, 1, 0, 0, 0, F);
  sph(g, 0.66, mid, 0.02, 2.06, 0.04, 1.12, 1.04, 1.06, 0, 0, 0, F);
  sph(g, 0.5, mid, -0.52, 2.1, 0.16, 1, 1, 1, 0, 0, 0, F);
  sph(g, 0.48, mid, 0.5, 2.16, -0.2, 1, 1, 1, 0, 0, 0, F);
  sph(g, 0.42, mid, 0.06, 2.5, -0.34, 1, 1, 1, 0, 0, 0, F);
  sph(g, 0.38, lite, 0.2, 2.62, 0.14, 1, 1, 1, 0, 0, 0, F);
  sph(g, 0.32, lite, -0.26, 2.56, 0.3, 1, 1, 1, 0, 0, 0, F);
  sph(g, 0.24, hi, 0.06, 2.9, 0.02, 1, 1, 1, 0, 0, 0, F);          // 顶部高光小球
  sph(g, 0.18, hi, -0.36, 2.76, -0.12, 1, 1, 1, 0, 0, 0, F);
  return g;
};
...
export const pine = (s = 1) => {
  const g = G();
  cyl(g, 0.16, 0.2, 0.26, '#6E5236', 0, 0.13, 0, 0, 0, 0, 10);      // 根部外扩
  cyl(g, 0.08, 0.13, 0.8, '#8A6844', 0, 0.4, 0, 0, 0, 0, 10);
  // 四层锥：暗/中交替（底部最暗）+ 刻面，顶上小一层提亮，轮廓才不是"叠了三个三角"
  const tones = ['#3E7A46', '#4E9152', '#5CA85C', '#7CC96F'];
  for (let i = 0; i < 4; i++) {
    const r = 0.82 - i * 0.15;
    cone(g, r, 0.92 - i * 0.06, tones[i], 0, 1.02 + i * 0.5, 0, 0, 0, 0, 10, { flat: true });
    cone(g, r * 0.98, 0.1, tones[Math.max(0, i - 1)], 0, 1.0 + i * 0.5 - 0.45, 0, 0, 0, 0, 10);   // 层间暗沿
  }
  sph(g, 0.16, '#8FD08F', 0, 3.06, 0, 1, 1.15, 1, 0, 0, 0, { flat: true });
  g.scale.setScalar(s);
  return g;
};
...
export const bush = (s = 1) => {
  const g = G();
  const F = { flat: true };
  sph(g, 0.4, '#3E7A46', 0, 0.24, 0, 1.25, 0.8, 1.05, 0, 0, 0, F);          // 底部暗团
  sph(g, 0.34, '#4E9152', 0.34, 0.26, 0.12, 1, 1, 1, 0, 0, 0, F);
  sph(g, 0.3, '#4E9152', -0.32, 0.24, -0.08, 1, 1, 1, 0, 0, 0, F);
  sph(g, 0.32, '#6FBF73', 0.06, 0.42, 0.04, 1, 1, 1, 0, 0, 0, F);           // 顶部亮团
  sph(g, 0.22, '#8FD08F', -0.22, 0.48, 0.2, 1, 1, 1, 0, 0, 0, F);
  sph(g, 0.18, '#8FD08F', 0.3, 0.5, -0.16, 1, 1, 1, 0, 0, 0, F);
  g.scale.setScalar(s);
  return g;
};
