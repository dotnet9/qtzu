...
PETS.cat = () => {
  ...
  // 胡须：每侧三根细圆柱（一眼认出是猫；原来只有鼻子与耳朵）
  for (const sx of [-1, 1]) for (let i = 0; i < 3; i++)
    cyl(head, 0.004, 0.004, 0.2, '#FFF6EC', 0.13 * sx, -0.02 + i * 0.028, 0.13, 0, 0, sx * (Math.PI / 2 + 0.18 - i * 0.12), 4);
...
PETS.dog = () => {
  ...
  // 项圈：一圈细环 + 小铃铛（家犬的识别件）
  tor(head, 0.115, 0.016, '#D95555', 0, -0.14, 0, Math.PI / 2, 0, 0, Math.PI * 2);
  sph(head, 0.022, '#E8C86A', 0, -0.15, 0.115);
  return g;
};
...
PETS.mouse = () => {
  ...
  for (const sx of [-1, 1]) for (let i = 0; i < 3; i++)
    cyl(head, 0.003, 0.003, 0.15, '#D8D8E4', 0.09 * sx, -0.02 + i * 0.022, 0.09, 0, 0, sx * (Math.PI / 2 + 0.2 - i * 0.14), 4);
...
PETS.fox = () => {
  ...
  for (const sx of [-1, 1]) for (let i = 0; i < 2; i++)
    cyl(head, 0.0035, 0.0035, 0.17, '#FFF6EC', 0.1 * sx, -0.02 + i * 0.03, 0.13, 0, 0, sx * (Math.PI / 2 + 0.16 - i * 0.16), 4);
  for (const sx of [-1, 1]) {
...
PETS.squirrel = () => {
  ...
  // 抱一颗橡果（与熊猫抱竹子同理，但橡果更"松鼠"）：果身 + 壳帽 + 小尖
  sph(g, 0.045, '#C99A5F', 0.13, 0.14, 0.1, 1, 1.2, 1);
  sph(g, 0.048, '#8A6844', 0.13, 0.175, 0.1, 1.05, 0.6, 1.05);
  cone(g, 0.012, 0.03, '#6B5236', 0.13, 0.205, 0.1);
  return g;
};
