...
export function buildPlayer(gender = 'boy', wear = {}) {
  ...
  for (const [leg, sx] of [[legL, -1], [legR, 1]]) {
    // 裤腿长在腿上（跟腿一起摆），不再由 body 上一根大圆柱代替 —— 否则两条腿会被裤身吞掉
    // 胶囊加长到 len 0.19 / r 0.055：腿顶到 world 0.29，才能和短裤（0.26 起）重叠。
    cap(leg, 0.055, 0.19, girl ? skin : '#7EA8E8', 0, -0.16, 0);
    if (!girl) cyl(leg, 0.06, 0.064, 0.05, '#6FA05A', 0, -0.245, 0, 0, 0, 0, 12);   // 裤脚翻边
    cyl(leg, 0.058, 0.058, 0.02, '#FFFBF2', 0, -0.30, 0.02, 0, 0, 0, 12);           // 鞋底白边（细节）
  ...
  if (girl) {
    // 女孩：上身 + **两层裙摆**（有层次，不是单锥台）+ 腰带 + 白领口
    cyl(body, 0.128, 0.152, 0.24, clothes, 0, 0.30, 0, 0, 0, 0, 16);     // 上身（直筒，下探到 0.48）
    cyl(body, 0.175, 0.235, 0.11, clothes, 0, 0.155, 0, 0, 0, 0, 18);    // 内层裙摆
    cyl(body, 0.215, 0.30, 0.13, clothes, 0, 0.055, 0, 0, 0, 0, 20);     // 外层裙摆（往下延到 0.29，盖住腿顶）
    cyl(body, 0.242, 0.247, 0.022, '#FFFFFF', 0, -0.005, 0, 0, 0, 0, 20); // 裙摆白边（层次感）
    cyl(body, 0.16, 0.163, 0.03, '#FFFFFF', 0, 0.215, 0, 0, 0, 0, 18);   // 腰带
    cyl(body, 0.1, 0.125, 0.06, '#FFFFFF', 0, 0.425, 0, 0, 0, 0, 16);    // 白色小领子
  } else {
    // 男孩：短裤 + 上衣 + 背带裤配件（裤腿在 legs 上，见上）
    // ⚠ 旧版把短裤放在 world 0.48~0.60、上衣放在 0.64~0.78 —— 而腿顶只到 0.282，
    //   中间 0.28~0.48 是空的，且上衣整颗被头球包住（头球 0.584~0.936）。
    //   现在短裤 0.26~0.46、上衣 0.445~0.605，逐段重叠，头球也接上。
    cyl(body, 0.152, 0.138, 0.20, clothes, 0, 0.06, 0, 0, 0, 0, 16);     // 短裤（上宽下窄，盖住腿顶 0.04）
    cyl(body, 0.158, 0.162, 0.035, '#5C8F46', 0, 0.1575, 0, 0, 0, 0, 16); // 腰带（绿）
    sph(body, 0.142, '#FFF6C8', 0, 0.225, 0, 1.04, 0.56, 0.94);          // 上衣（浅黄，压扁贴合躯干）
    cyl(body, 0.146, 0.149, 0.03, '#F5E9A8', 0, 0.175, 0, 0, 0, 0, 16);  // 衣摆（深一档，盖住腰带接缝）
    box(body, 0.07, 0.30, 0.03, '#5C8F46', -0.075, 0.16, 0.128);         // 背带（从裤腰跨到肩）
    box(body, 0.07, 0.30, 0.03, '#5C8F46', 0.075, 0.16, 0.128);
    box(body, 0.115, 0.09, 0.02, '#6FA05A', 0, 0.02, 0.152);             // 前口袋（在短裤上）
    box(body, 0.075, 0.02, 0.02, '#4E7A3C', 0, 0.06, 0.154);             // 口袋口
    for (const sx of [-1, 1]) sph(body, 0.016, '#8A6AC4', 0.055 * sx, 0.11, -0.135);  // 背后扣子（细节）
  }
  // 手臂
  const armL = G(), armR = G();
  ...
  for (const sx of [-1, 0, 1]) {
    sph(head, 0.05, '#A6704A', 0.055 * sx + 0.01, 0.126 - Math.abs(sx) * 0.012, 0.108, 1.5, 0.5, 0.8);
  }
  if (girl) {
    // 双马尾 + 粉色发圈 + 头顶蝴蝶结
    for (const sx of [-1, 1]) {
      // 马尾加长到肩（原来 1.15 只是个小揪，与男孩剪影区分不够）
      sph(head, 0.082, '#8A5A3C', 0.19 * sx, -0.045, -0.02, 0.82, 1.85, 0.85);
    ...
    // 草帽（蓝色帽带）：戴了商店帽就摘下
    // 帽檐原来半径 0.26（比头半径 0.185 宽 40%），俯视就是"一顶大帽子盖住整个人"
    cyl(head, 0.135, 0.205, 0.05, '#F5D76E', 0, 0.152, 0, 0, 0, 0, 18);      // 帽檐（收窄 21%）
    cyl(head, 0.20, 0.205, 0.014, '#E0BE52', 0, 0.13, 0, 0, 0, 0, 18);       // 帽檐下沿（有厚度，不是一张纸）
    sph(head, 0.128, '#F5D76E', 0, 0.19, 0, 1, 0.78, 1);                     // 帽顶（更高更圆）
    cyl(head, 0.14, 0.143, 0.036, '#7EC4F2', 0, 0.176, 0, 0, 0, 0, 18);      // 蓝帽带
  }
  // ---- 商店装扮：帽子 ----
  if (wear.hat === 'wizard') {
  ...
  } else if (wear.hat === 'helmet') {
    // 赛车头盔（呼应参考图）：圆盔 + 白条 + 护目镜 + 下巴带
    sph(head, 0.205, '#E0534B', 0, 0.12, 0, 1.02, 0.92, 1.02);
    cyl(head, 0.208, 0.212, 0.03, '#F5F1E8', 0, 0.135, 0, 0, 0, 0, 18);      // 白条
    cyl(head, 0.2, 0.215, 0.075, '#3A4450', 0, 0.075, 0.02, Math.PI / 2, 0, 0, 18);  // 护目镜框
    sph(head, 0.155, '#8ED8F8', 0, 0.075, 0.06, 1, 0.55, 0.5, 0, 0, 0, { rough: 0.25, emissive: '#4E9EE8', ei: 0.15 }); // 镜片
    cyl(head, 0.012, 0.012, 0.1, '#F5F1E8', -0.14, -0.05, 0.1, 0, 0, 0.35, 6);       // 下巴带
    cyl(head, 0.012, 0.012, 0.1, '#F5F1E8', 0.14, -0.05, 0.1, 0, 0, -0.35, 6);
    sph(head, 0.022, '#FFD34E', 0.17, 0.16, -0.02, 1, 1, 1, 0, 0, 0, { emissive: '#FFD34E', ei: 0.4 });
  } else if (wear.hat === 'crown') {
    // 小皇冠：金环 + 5 个尖 + 宝石
    cyl(head, 0.17, 0.18, 0.07, '#F0C44E', 0, 0.175, 0, 0, 0, 0, 18);
    cyl(head, 0.185, 0.185, 0.022, '#FFE08A', 0, 0.145, 0, 0, 0, 0, 18);
    for (let i = 0; i < 5; i++) {
      const a = Math.PI * 2 * i / 5;
      cone(head, 0.045, 0.13, '#F0C44E', Math.cos(a) * 0.155, 0.26, Math.sin(a) * 0.155, 0, 0, 0, 4);
    }
    for (let i = 0; i < 5; i++) {
      const a = Math.PI * 2 * i / 5 + Math.PI / 5;
      sph(head, 0.022, i % 2 ? '#FF6F8F' : '#6FC8F0', Math.cos(a) * 0.17, 0.19, Math.sin(a) * 0.17,
        1, 1, 1, 0, 0, 0, { emissive: i % 2 ? '#FF6F8F' : '#6FC8F0', ei: 0.45 });
    }
  } else if (wear.hat === 'chef') {
    // 厨师帽：白帽箍 + 蓬松帽体 + 顶部小折
    cyl(head, 0.185, 0.19, 0.08, '#F5F1E8', 0, 0.17, 0, 0, 0, 0, 18);
    sph(head, 0.2, '#FFFFFF', 0, 0.29, 0, 1.05, 0.85, 1.05);
    sph(head, 0.12, '#FFFFFF', -0.09, 0.4, 0.05);
    sph(head, 0.115, '#FFFFFF', 0.1, 0.41, -0.03);
    sph(head, 0.1, '#F0EDE6', 0.02, 0.46, 0.06);
  }
  // ---- 商店装扮：气球（拴在右手，走路一颠一颠） ----
  let balloon = null;
  if (wear.balloon) {
