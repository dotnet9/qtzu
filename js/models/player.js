...
export function buildPlayer(gender = 'boy', wear = {}) {
  ...
  for (const [leg, sx] of [[legL, -1], [legR, 1]]) {
    ...
    cap(leg, 0.052, 0.16, girl ? skin : '#7EA8E8', 0, -0.15, 0);
    if (!girl) cyl(leg, 0.058, 0.062, 0.05, '#6FA05A', 0, -0.235, 0, 0, 0, 0, 12);  // 裤脚翻边
  ...
  if (girl) {
    ...
    cyl(body, 0.125, 0.155, 0.22, clothes, 0, 0.31, 0, 0, 0, 0, 16);     // 上身（直筒）
    cyl(body, 0.175, 0.235, 0.1, clothes, 0, 0.16, 0, 0, 0, 0, 18);      // 内层裙摆
    cyl(body, 0.215, 0.3, 0.1, clothes, 0, 0.07, 0, 0, 0, 0, 20);        // 外层裙摆（更外扩、更短）
    cyl(body, 0.24, 0.245, 0.022, '#FFFFFF', 0, 0.025, 0, 0, 0, 0, 20);  // 裙摆白边（层次感）
    cyl(body, 0.16, 0.163, 0.03, '#FFFFFF', 0, 0.215, 0, 0, 0, 0, 18);   // 腰带
    cyl(body, 0.1, 0.125, 0.06, '#FFFFFF', 0, 0.425, 0, 0, 0, 0, 16);    // 白色小领子
  } else {
    // 男孩：胯部 + 上衣 + 背带裤配件（裤腿在 legs 上，见上）
    cyl(body, 0.15, 0.165, 0.12, clothes, 0, 0.24, 0, 0, 0, 0, 16);      // 胯部（短，露出裤腿）
    cyl(body, 0.168, 0.172, 0.035, '#5C8F46', 0, 0.30, 0, 0, 0, 0, 16);  // 腰带（绿）
    sph(body, 0.128, '#FFF6C8', 0, 0.41, 0, 1.06, 0.55, 0.92);           // 上衣（浅黄，略宽肩：与女孩的窄肩区分）
    box(body, 0.06, 0.22, 0.03, '#5C8F46', -0.075, 0.38, 0.135);         // 背带（加宽到 0.06）
    box(body, 0.06, 0.22, 0.03, '#5C8F46', 0.075, 0.38, 0.135);
    box(body, 0.115, 0.09, 0.02, '#6FA05A', 0, 0.28, 0.165);             // 前口袋
    box(body, 0.075, 0.02, 0.02, '#4E7A3C', 0, 0.32, 0.167);             // 口袋口
  }
  // 手臂
  const armL = G(), armR = G();
