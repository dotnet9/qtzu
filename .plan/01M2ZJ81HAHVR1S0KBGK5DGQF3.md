...
## 十、实施记录（B 段：成都地面烘焙，本轮完成）

### 10.1 已落地并实测

| 项 | 实测 |
|---|---|
| 烘焙产物 | `assets/models/ground/58c74cc3.glb`：三角 16308 / 顶点 13623 / 1444KB（预算 60000 / 4.5MB） |
| 可复现性 | `run.mjs --kinds ground --city chengdu --verify` 二次烘焙哈希一致 ✓ |
| 审计 | `audit-assets.mjs` **0 缺失 / 0 孤儿 / 0 超预算（524 个）**；单城成都 3.82MB < 8MB |
| 渲染门槛 | `check-render.mjs` **全部达标**（过曝 0.002 / 上缘 0.784 / 草地 亮 0.607 饱 0.414 / 开闭后期差 0.005） |
| 贴地一致性 | `verify-ground-fit.mjs` **最大偏差 0.030 / 均值 0.011**（门槛 0.12），42/42 格点命中 |
| 换装/回退 | `test-fallback.mjs` 两趟全绿：回退趟 ground-slot 空 + 程序化地面可见（不是空城）；正常趟 slot 有子节点 + 程序化地面已隐藏（不双向重叠） |
| 性能 | 帧时长中位数 16.70ms（与改造前持平）；绘制调用 369 / 三角 145.3k |

### 10.2 本轮修掉的 3 个真 bug（此前该段一直未能烘出产物）

1. **`terrain-lib.mjs::radiusOf` 口径错**（根因）：读 `city.json` 的 unis/foods/scenes，而它们在独立文件里
   → n 恒为 0 → 成都半径 76 vs 游戏实际 87 → 规格网格 102×72 对不上运行时 116×83，
   烘焙地面比城小一圈、高度整体错位（贴地校验实测最大偏差 7.19）。修后 87 ✓，偏差降到 0.030 ✓
2. **`ground.py::_miter_normals` 少返回一个法线**（闭合轮廓末尾=首点）→ 首次烘焙 IndexError。
3. **`ground.py::wall` 垛口定位 `t` 未钳制** → 近重合点让 t 变天文数字，垛口被甩到岛外
   （实测墙顶 y≈3 的顶点跑到 x=−113.6，轮廓只有 ±76）—— 正是 §五 B4.2 那条 bbox 断言抓到的。

### 10.3 已知取舍（需要决策）

烘焙地面是**顶点色 + AO、零贴图**（本方案 §九 的原始决策）。但 A 段（渲染纠错那轮）已经给程序化地面
加了平铺细节贴图 + 区域色贴图 + 地面散点；换装后程序化地面被隐藏，**那些贴图细节在近景会丢失**。
实测差异（同机位左右对比）：草地饱和 0.465 → 0.414、亮度 0.594 → 0.607（仍在门槛内，但确实淡了一点）。
补偿：院墙/垛口/岩裙/雪峰现在有了烘焙 AO 与更干净的几何。

两条路：
- **A** 给 ground.py 加内嵌贴图（1024² albedo 由规格逐格色 + 细节噪声生成、512² 平铺法线），
  材质拆成"贴图地面 + 顶点色部件"两份；体积 +约 1MB/城。这是用户批准过的口径。
- **B** 保持零贴图（更省体积），把 A 段那份平铺细节贴图**改挂到烘焙地面的材质**上（运行时不烘、复用现有贴图）。

### 10.4 未做

- 其余 51 城地面烘焙（`run.mjs --kinds ground` 不带 `--city`；需先逐城核 radiusOf / 规格 / 预算）。
- 同机位对比图：`.cache/look/{A-ground-default,B-ground-city,C-ground-closeup}.png`（左=程序化，右=烘焙）。

### 10.5 全量铺开（52 城，本轮完成）

| 项 | 实测 |
|---|---|
| 产物 | 52 城地面 GLB，合计 123.4MB / 705k 三角面；色图全部走 TEXCOORD_1（逐个校验 GLB 结构） |
| 审计 | 575 个资产 **0 缺失 / 0 孤儿 / 0 超预算**；单城首屏最大北京 7.29MB（上限 8MB） |
| 巡城（分两段） | 52 城 **386/386 校门、139/139 地标、0 错误、0 个 404 资源** |
| 贴地一致性 | 成都 0.030 / 北京 0.053 / 拉萨 0.036（门槛 0.12） |
| 回退验收 | test-fallback 两趟全绿（含地面换装断言：回退趟画卷仍在、正常趟不双向重叠） |
| 性能 | 帧时长中位数 16.80ms；373 draw calls / 162.8k 三角面 |

本轮修掉的三个 bug（都是"地面加入队列后"才暴露的）：

1. **超时 = 永久回退**：apply() 在 1.5s 超时后返回 false 就不再管；地面 GLB 有 2~3MB 占着队列，
   排队靠后的校门超时后**永久停在程序化形态**（巡城命中率掉到 80%，而失败清单是空的——它不算"失败"）。
   修法：超时组登记进 lateSwap，资产真正到货后 drainLate() 补换（只在 group 仍挂在场景里时换）；
   并发 4→6、超时 1500→2000ms。
2. **A1 色图 UV 两层错**（用户看图发现"右侧观感很差"）：先是被平铺 UV 重复成马赛克；
   修完仍错在 V 轴取 `co.z`（Blender 里那是**高度**，水平深度是 `co.y`）→ 整城贴着贴图里
   一条水平线采样。两处都修好后：细节能量 -24% → -8.4%、饱和 -5% → +4.9%。
3. **验收脚本的测量方法错**（不是产品坏）：巡城等待从"固定睡眠 4.5s"改成
   "等该城换装数量达到 manifest 里的期望值"（固定睡眠会量到假低命中率：北京 9/30，等够就是 30/30；
   `busy()===0` 也不行——换城后请求是异步排队的，请求还没发起时它也是 0）；
   verify-ground-fit 兼容"全量合并规格 ground.all.json"。

同机位对比图：`.cache/look/{1-final-street,2-final-city,3-final-floor,4-final-gate}.png`（左=改造前/程序化，右=现在）。
